...
import 'package:MMLMobile/screens/navigation/videoTrimmer/thumbnail_viewer.dart';
import 'package:MMLMobile/screens/navigation/videoTrimmer/trim_editor_painter.dart';
...
class TrimEditor extends StatefulWidget {
  ...
  final Function(double startValue, Offset startPos, Offset endPos)
      onChangeStart;

  /// Callback to the video end position.
  ///
  /// Returns the selected video end position in `milliseconds`.
  final Function(double endValue, Offset startPos, Offset endPos) onChangeEnd;
  ...
  final double defaultStartValue;
  final double defaultEndValue;
...
class _TrimEditorState extends State<TrimEditor> with TickerProviderStateMixin {
  ...
  Offset _endPos = Offset(300.0, 50.0);
  ...
  Future<void> _initializeVideoController() async {
    if (_videoFile != null) {
      ...
      widget.onChangeEnd(_videoEndPos, _startPos, _endPos);
      ...
      if (maxLengthPixels != null) {
        if (!(_endPos.dx - _startPos.dx - details.delta.dx > maxLengthPixels)) {
          setState(() {
            ...
            print("Video Start Position");
            print(_videoStartPos);
            widget.onChangeStart(_videoStartPos, _startPos, _endPos);
          });
  ...
  void initState() {
    ...
    if (widget.defaultStartValue != null && widget.defaultEndValue != null) {
      if (widget.defaultStartValue > 0 && widget.defaultEndValue > 0) {
        double videoLength = totalDuration.inMilliseconds.toDouble();
        print("video Length");
        print(videoLength);
        double start_dx =
            ((widget.defaultStartValue / videoLength) * widget.viewerWidth);
        double end_dx =
            ((widget.defaultEndValue / videoLength) * widget.viewerWidth);
        print("Start dx");
        print(start_dx);
        print("End dx");
        print(end_dx);
        setState(() {
          _startPos = Offset(start_dx, 0.0);
          _endPos = Offset(end_dx, _thumbnailViewerH);
        });
      } else {
        setState(() {
          _endPos = Offset(
